...
## 你的任务
1. 阅读以下源代码目录中的文件：{source_dirs}
   - 如果配置了 focus_paths，优先深入分析这些路径：{focus_paths}
   - 如果配置了 exclude_paths，跳过这些路径：{exclude_paths}
...
## 改进建议（按优先级排列）
## 关键代码索引
（列出本轮分析中涉及的关键文件路径、核心函数/组件名、主要依赖关系，供后续 Agent 精准定位）
